function viewModel() {
	...
	function getMatches() {
		...
		sqEventProxy.getMatchesByClub(
			{ clubShort: 'LSQ', period : 'ranking' },
			function(data) {
				setTimeout(function() {
					ko.mapping.fromJS(data.scores, {}, self.matchList);
					ko.mapping.fromJS(data.scores, {}, self.list);
	...
	self.droppedMatches = ko.observableArray();

	function getDroppedMatches() {
		var data;
		sqEventProxy.getDroppedMatches(
			{ data : data },
			function(data) {
				if(data.matches) {
					ko.mapping.fromJS(data.matches, {}, self.droppedMatches);
				}
			}
		);
	}
